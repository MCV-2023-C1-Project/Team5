...
class Histogram:
    ...
    def __call__(self, img, mask=None):
        ...
        histogram = np.vstack(
            [cv2.calcHist([img], [channel], mask, [self.kwargs["bins"]],
                          self.kwargs["range"])
...
class SpatialDescriptor:
    ...
    def __call__(self, img, mask=None):
        if mask is None:
            ...
            tile_descriptors = [self.base_descriptor(img_tile, mask_tile)
                                for img_tile, mask_tile in\
                                    zip(self.split_in_tiles(img),
                                        self.split_in_tiles(mask))]           
