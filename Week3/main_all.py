...
# set paths
QUERY_IMG_DIR = Path(os.path.join("data", "qsd1_w3"))
REF_IMG_DIR = Path(os.path.join("data", "BBDD"))
RESULT_OUT_PATH = Path(os.path.join("all_qsd2_K5.pkl"))
...
TEXTURE_DESCRIPTOR_1 = SpatialDescriptor(
    DiscreteCosineTransform(num_coeff=4), SPLIT_SHAPE
)
COLOR_DESCRIPTOR_1 = SpatialDescriptor(
    Histogram(color_model="yuv", bins=25, range=(0, 255)), SPLIT_SHAPE
)
K = 10
...
Similar_Artist = ArtistReader(
    TEXT_DETECTOR, path_bbdd_csv=path_csv_bbdd, save_txt_path=path_txt_artists
)
    ...
    if v2:
        ...
        for i, img in enumerate(imgs):
            Image.fromarray(img).save(
                Path(os.path.join(denoised_path, "{}_{}.png".format(idx, i)))
            )
            text_mask = TEXT_DETECTOR.get_text_mask(img)
            set_images_color.append(
                COLOR_DESCRIPTOR_1(img, text_mask)
            )  # add "idx: descriptor" pair
            set_images_texture.append(
                TEXTURE_DESCRIPTOR_1(img, text_mask)
            )  # add "idx: descriptor" pair
...
if v2:
    ...
    if with_text_combination:
        ...
        result = Compare_Artist.compare_multiple_paintings_artist(
            result_dict, query_artist
        )
    else:
        """
